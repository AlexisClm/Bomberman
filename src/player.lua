...
local function initPlayer()
  data.sprite = love.graphics.newImage("Assets/Images/Player.png")
  data.w      = 40
  data.h      = 62
  data.x      = grid.getX(2)+10
  data.y      = grid.getY(2)-20
  data.speed  = 150
  data.hp     = 1
  data.alive  = true
  data.state  = true
...
local function checkWin()
  if (gameState.getState() == "gameSolo") then
    if (data.line == 6) and (data.column == 12) then
      gameState.setState("gameWin")
      sound.getWin():play()
...
function class.keypressed(key)
  if (key == "space") and (bomb.getNbBombs() < bomb.getNbBombsMax()) and (not grid.isBomb(data.line, data.column)) and (data.alive) then
    bomb.initBomb(data.line, data.column)
    grid.setBomb(data.line, data.column, true)
    sound.getBomb():play()
  elseif (key == "escape") then
    if (gameState.getState() == "gameSolo") then
      gameState.setState("pauseSolo")
    elseif (gameState.getState() == "gameDuo") then
      gameState.setState("pauseDuo")
    end
    sound.getBackground():setVolume(0.2)
  end
end
